...
GitHub allows you to archive and label a specific version of your repository in the form of a release. A release tags a particular commit with an arbitrary label, e.g., as "submission,"[^sub] "preprint," or "published,"[^pub] and archives also "binary" products of your code, e.g. the resulting pdf of the manuscript or the docker image (see [Containerization]). From such a release, [zenodo.org](https://zenodo.org) or [figshare.com](https://figshare.com) can create a DOI, making it easier to reference and retrieve it (see the [GitHub Guide](https://guides.github.com/activities/citable-code/)[^doi]).
...
[^sub]: We created an release for the submission: [https://github.com/aaronpeikert/reproducible-research/releases/tag/v0.1.1-submission](https://github.com/aaronpeikert/reproducible-research/releases/tag/v0.2.1-submission)
[^pub]: We created an release for the publication: [https://github.com/aaronpeikert/reproducible-research/releases/latest](https://github.com/aaronpeikert/reproducible-research/releases/latest)
...
If you have followed our workflow as presented thus far, you are (almost) only three commands away from fully reproducing our paper. You simply have to type the following commands on the command line:
...
# the package version if fixed by hash (user/package@hash)
RUN installGithub.r\
...
The `FROM` statement specifies which Docker image to use, in this case, the `rocker/verse` image with the tag 3.6.1 (referring to the R version 3.6.1). The `RUN` statement describes a command to execute, in this case, to run an R script `install2.r` which is available on all Rocker images, to install the specified packages (here, `pacman`,  `here` & `pander`). A Dockerfile allows more than one `RUN` statement, executing arbitrary system commands. Those `RUN` statements can install dependencies that are not an R package, e.g., other programming languages like python or Matlab. The `WORKDIR` statement is not strictly necessary but simplifies commands. The command `docker build -t image-name` creates an image named `image-name` from the Dockerfile in the project. A way to identify the dependencies automatically and generate a docker image from them is provided in the [`liftR`](https://cran.r-project.org/web/packages/liftr/vignettes/liftr-intro.html) package [@liftr].
...
[^home]: See the section on [related approaches] for alternatives to Docker in case you cannot avoid Windows Home.
