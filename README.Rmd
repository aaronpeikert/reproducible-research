...
**Note: `make build` no longer works because MRAN is permanently offline. Instead, download the pre-built Docker image from the release:**
...
**Note: `make rebuild` no longer works because MRAN is permanently offline. Use `docker load` as shown above to restore the image.**
